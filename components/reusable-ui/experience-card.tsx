    ...
    return(
        <div className={`w-90/100 flex justify-between flex-wrap items-center gap-6 ${index%2 != 0 ? 'flex-row-reverse':'flex-row'}`}>
            <div className="w-full md:w-1/2 flex flex-col gap-3">
                    <div className="flex flex-col gap-3">
                        ...
                        <div className="flex flex-col gap-0.5">
                            ...
                            <h3 className="text-base md:text-lg font-semibold text-lightBlue">{data.companyName}</h3>
                            <h5 className="text-sm md:text-base text-slate-300 my-1">{data.period}</h5>
                        </div>
                    </div>
                    <ul className="flex flex-col gap-2 list-disc pl-5 marker:text-lightBlue">
                        {data.info.map((point, i) => (
                            <li key={i} className="text-white text-xs md:text-base font-medium leading-relaxed">{point}</li>
                        ))}
                    </ul>
            ...
            <div className="w-full md:w-45/100 h-full overflow-hidden rounded-lg">
                <Image className="w-full h-auto object-cover transform transition-transform duration-500 ease-in-out hover:scale-110 hover:-translate-x-4" width={560} height={400} src={data.imgLink} alt={`${data.companyName} — ${data.internRole}`}/>
            </div>
